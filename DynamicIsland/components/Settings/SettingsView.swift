...
private enum SettingsSearchIndex {
    static let entries: [SettingsSearchEntry] = [
        ...
        SettingsSearchEntry(tab: .extensions, title: "Browse the Marketplace", keywords: ["marketplace", "extensions", "install", "download", "store", "getvone"], highlightID: SettingsTab.extensions.highlightID(for: "Browse the Marketplace")),
...
/// running, so quitting the provider gives them back to Vone -- and a control
...
private struct ExternalDisplayIntegrationsSection: View {
    ...
    /// provider is quit, Vone handles the keys again and the saved step sizes
    ...
    private var providerStatusDescription: String {
        ...
        case .betterDisplay:
            if !betterDisplayManager.isDetected {
                return "Install [BetterDisplay](https://betterdisplay.pro) to control external display brightness (and optional volume) through Vone's HUD."
            }
            ...
            return "BetterDisplay OSD events will be routed through Vone's active HUD style. Brightness is always routed; volume is routed when external volume control listener is enabled below. Make sure BetterDisplay's OSD integration is enabled in Settings › Application › Integration."
        case .lunar:
            if !lunarManager.isDetected {
                return "Install [Lunar](https://lunar.fyi) to control external display brightness, contrast, and optional volume through Vone's HUD via DDC."
            }
            if !ddcProviderRunning {
                return "Lunar is installed but not currently running. Launch Lunar to enable integration."
            }
            if lunarManager.isConnected {
                return "Connected to Lunar's DDC socket. Brightness and contrast adjustments are shown through Vone's HUD; volume follows when external volume control listener is enabled below."
            }
            ...
            Section {
                ...
                if enableThirdPartyDDCIntegration {
                    ...
                    Text(
                        ...
                        ? "Vone's built-in volume key interception is disabled while external volume listening is on. Volume HUD/OSD will follow \(thirdPartyDDCProvider.displayName) payloads."
                        : "Vone keeps native volume key interception. External provider volume payloads are ignored while this is off."
                    )
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    ...
                    Text("Enable to route BetterDisplay or Lunar display adjustments through Vone's active HUD style.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            } footer: {
                if enableThirdPartyDDCIntegration {
                    Text("Vone always listens to selected-provider brightness events, and listens to provider volume events only when external volume listener is enabled.")
                        .foregroundStyle(.secondary)
                        .font(.caption)
                }
            }
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text("Gives each app its own volume and mute, independent of the system volume. macOS will ask for audio recording permission the first time an app's level is changed: routing an app's audio through Vone is the only way it can be adjusted, and nothing is ever recorded or written to disk. Apps left at 100% and unmuted keep their normal, untouched path to the speakers.")
            }
        ...
        Form {
            ...
            Section {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Right-click the album art on the lock screen to set it as the wallpaper. Right-click again or click the background to restore the original wallpaper. If a canvas is available, Vone can also keep the same album art + player layout on top of the live canvas.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
...
struct StatsSettings: View {
    ...
    var body: some View {
        ...
        .confirmationDialog(
            ...
            Text("This removes \(account.name) and its stored API key from Vone.")
        }
        ...
        Form {
            ...
            if enableNotes {
                Section {
                    ...
                    Text("Two-way sync with the macOS Notes app. Notes created in Vone appear in the Vone folder in Notes, and your existing Apple Notes are imported into the notch. Grant Automation permission for Notes when prompted.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
