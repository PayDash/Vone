...
/// Shared constants for the Vone extension XPC service.
...
final class ExtensionXPCServiceHost: NSObject, NSXPCListenerDelegate {
    ...
    func start() {
        ...
        if AppRuntimeEnvironment.isUITesting {
            Logger.log("Bypassing Vone XPC listener for UI testing", category: .extensions)
            return
        }
        ...
        Logger.log("Started Vone XPC listener", category: .extensions)
    }
    ...
    func stop() {
        ...
        Logger.log("Stopped Vone XPC listener", category: .extensions)
    }
