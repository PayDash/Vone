...
struct DynamicNotchApp: App {
    ...
    private let updaterDelegate = VoneUpdaterDelegate()

    init() {
        // Skip Sparkle's launch-time update check during UI testing.
        // The VoneUpdaterDelegate overrides the feed URL at runtime
...
//    private var windowSizeUpdateWorkItem: DispatchWorkItem?
    ...
    func applicationWillTerminate(_ notification: Notification) {
        let userInfo: [String: Any] = [
            VoneDistributedNotifications.UserInfoKey.sourcePID: NSNumber(value: ProcessInfo.processInfo.processIdentifier)
        ]
        DistributedNotificationCenter.default().postNotificationName(
            VoneDistributedNotifications.didBecomeIdle,
            object: nil,
            userInfo: userInfo,
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DistributedNotificationCenter.default().postNotificationName(
            VoneDistributedNotifications.didBecomeActive,
            object: nil,
            userInfo: userInfo,
            deliverImmediately: true
        )
    ...
    private func installTopMenuItemsIfNeeded() {
        ...
        if mainMenu.items.contains(where: { $0.identifier?.rawValue == "Vone.Focus.Menu" }) {
        ...
        focusMenuItem.identifier = NSUserInterfaceItemIdentifier("Vone.Focus.Menu")
        ...
        accessibilityMenuItem.identifier = NSUserInterfaceItemIdentifier("Vone.Accessibility.Menu")
        ...
        permissionsMenuItem.identifier = NSUserInterfaceItemIdentifier("Vone.Permissions.Menu")
        ...
        toolsMenuItem.identifier = NSUserInterfaceItemIdentifier("Vone.Tools.Menu")
        ...
        loggingLevelItem.identifier = NSUserInterfaceItemIdentifier("Vone.Tools.LoggingLevel")
    ...
    @objc private func setLogLevel(_ sender: NSMenuItem) {
        ...
        guard let mainMenu = NSApp.mainMenu,
              let toolsItem = mainMenu.items.first(where: { $0.identifier?.rawValue == "Vone.Tools.Menu" }),
              let toolsMenu = toolsItem.submenu,
              let loggingItem = toolsMenu.items.first(where: { $0.identifier?.rawValue == "Vone.Tools.LoggingLevel" }),
    ...
    @objc private func exportLogs() {
        ...
        savePanel.nameFieldStringValue = "Vone_Logs.zip"
        ...
        savePanel.begin { response in
            ...
            Task.detached(priority: .utility) {
                do {
                    ...
                    for file in allFiles where file.lastPathComponent.contains("Vone") {
                    ...
                    for file in sysFiles where file.lastPathComponent.contains("Vone") {
                        try? FileManager.default.copyItem(at: file, to: tempDir.appendingPathComponent(file.lastPathComponent))
                    }
