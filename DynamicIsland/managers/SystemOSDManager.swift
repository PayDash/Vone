...
class SystemOSDManager {
    ...
    /// so a SIGSTOP-frozen OSDUIHelper stays frozen after Vone quits, breaking
    /// every native OSD Vone does not replace (keyboard backlight,
    ...
    /// is resumed before Vone exits. Idempotent; safe to call from a
    /// termination handler.
