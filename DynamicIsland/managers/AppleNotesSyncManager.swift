...
enum AppleNotesSyncError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .automationDenied:
            return String(localized: "Allow Vone to control Notes in System Settings → Privacy & Security → Automation.")
...
final class AppleNotesSyncManager: ObservableObject {
    ...
    private static let syncFolderName = "Vone"
    private static let fieldSeparator = "\u{241F}"
    private static let recordSeparator = "\u{241E}"
    private static let voneTagPattern = #"<!--vone:id=([0-9A-Fa-f-]{36})-->"#
    ...
    private func merge(localNotes: [NoteItem], remoteNotes: [RemoteAppleNote]) async throws -> [NoteItem] {
        ...
        // 处理并占位，新副本被跳过，voneId 匹配与新笔记导入的结果就会依赖数组顺序。
        for remote in remoteById.values where !linkedRemoteIds.contains(remote.id) {
            if let voneId = remote.voneId,
               let index = notes.firstIndex(where: { $0.id == voneId }) {
    ...
    private func fetchRemoteNotes() async throws -> [RemoteAppleNote] {
        ...
        tell application "Notes"
            ...
            repeat with n in notes of default account
                if password protected of n is false then
                    ...
                    set voneId to my extractVoneId(body of n)
                    set chunk to (id of n) & fieldSep & (name of n) & fieldSep & ((creation date of n) - epoch as string) & fieldSep & ((modification date of n) - epoch as string) & fieldSep & notePlain & fieldSep & voneId
        ...
        on extractVoneId(noteBody)
            if noteBody does not contain "vone:id=" then return ""
            set AppleScript's text item delimiters to "vone:id="
    ...
    private func parseRemoteNotes(_ payload: String) -> [RemoteAppleNote] {
        ...
        return payload
            ...
            .compactMap { record in
                ...
                let voneId = fields.count > 5
                    ? UUID(uuidString: String(fields[5]))
                    : extractVoneId(from: content)
    ...
    private func extractVoneId(from content: String) -> UUID? {
        guard let regex = try? NSRegularExpression(pattern: Self.voneTagPattern) else { return nil }
    ...
    private func htmlBody(for note: NoteItem) -> String {
        ...
        return html + "<!--vone:id=\(note.id.uuidString)-->"
    }
