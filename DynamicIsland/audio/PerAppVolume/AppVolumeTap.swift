...
private let volumeTapLog = OSLog(subsystem: "com.ebullioscopic.Atoll.dynamicisland", category: "PerAppVolume")

/// Re-routes one app's audio through Vone so its level can be changed
...
final class AppVolumeTap {
    ...
    init(bundleIdentifier: String, processObjectIDs: [AudioObjectID], outputDeviceUID: String) throws {
        ...
        description.name = "Vone volume tap for \(bundleIdentifier)"
        ...
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey: "Vone-\(bundleIdentifier)",
    ...
    func activate() throws {
        let queue = DispatchQueue(label: "com.ebullioscopic.Atoll.pervolume.\(bundleIdentifier)", qos: .userInteractive)
