...
enum ClaudeKeychainTests {
    static func main() throws {
        ...
        let service = "Vone-keychain-regression-\(UUID().uuidString)"
        ...
        for cycle in 1...3 {
            ...
            try require(ClaudeKeychainStore.update(service: service, account: account, secret: updated) == nil, "Vone write \(cycle)")
            let actual = ClaudeKeychainStore.read(service: service, account: account)
            try require(actual == updated, "exact round trip \(cycle)")
            try require(try partitionIDs(service: service, account: account) == initialPartitions, "partitions survive Vone write \(cycle)")
        ...
        print("PASS: three Vone/owner round trips, unchanged partition permissions, exact payload, account isolation, missing/oversized/invalid input")
    }
